...
export default function ItemCarrinho({ item }) {
  ...
  return (
    <Row className="align-items-center">
      <Col xs="auto">
        <img
          src={`${import.meta.env.BASE_URL}images/${item.imgSrc}`}
          alt={item.titulo}
      ...
      <Col xs="auto" className="d-flex align-items-center gap-3">
        <Button variant="outline-danger" size="sm" onClick={diminuirQuantidade}>
          -
        </Button>
        ...
        <div className="text-center">
          ...
          <div>
            <b>
              Total: R$ {(parseFloat(item.valor) * item.quantidade).toFixed(2)}
            </b>
          </div>
        </div>
        <Button
